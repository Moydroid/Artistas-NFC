...
export default function Home() {
  ...
  const fetchArtists = async () => {
    const { data } = await supabase.from('artists').select('id, name, slug, cover_url, short_bio').eq('is_active', true).order('created_at', { ascending: false }).limit(6);
  ...
  const playPreview = async (artist: any) => {
    const { data: tracks } = await supabase.from('tracks').select('*').eq('artist_id', artist.id).limit(1);
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white overflow-x-hidden relative">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {artists.map(artist => (
              <div key={artist.id} className="group relative bg-zinc-900/50 backdrop-blur-md p-6 rounded-2xl border border-purple-500/20 hover:border-purple-500/50 transition-all">
                <img src={artist.cover_url || 'https://via.placeholder.com/400'} alt={artist.name} className="w-full h-48 object-cover rounded-xl mb-4" />
      ...
      <section className="py-32 px-4 text-center border-t border-purple-500/10 relative z-10 bg-[#0a0a0a]">
        <div className="max-w-3xl mx-auto">
          ...
          <a href={whatsappUrl} target="_blank" className="inline-flex items-center gap-3 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white px-10 py-5 rounded-full font-bold text-xl transition-all shadow-[0_0_40px_rgba(168,85,247,0.4)]">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-purple-500/30 rounded-3xl p-8 max-w-md w-full shadow-2xl text-center">
            ...
            <p className="text-zinc-300 mb-6">Eso fue solo un fragmento. <br /><span className="text-white font-bold">Escanea tu tarjeta FONOTAP</span> para escuchar la canción completa y acceder a contenido exclusivo.</p>
            <div className="flex flex-col gap-3">
              <a href={whatsappUrl} target="_blank" className="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white py-4 rounded-xl font-bold text-lg transition-all shadow-lg"> Quiero mis tarjetas</a>
              <button onClick={() => setShowModal(false)} className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 py-3 rounded-xl font-medium transition-colors">Seguir explorando</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-zinc-900 border border-purple-500/30 rounded-3xl p-8 max-w-md w-full shadow-2xl relative" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="bg-black/40 p-4 rounded-xl border border-zinc-700">
                <div className="flex items-center gap-3 mb-2"><span className="text-2xl">📲</span><h4 className="font-bold text-white">Opción 1: NFC (Recomendado)</h4></div>
              ...
              <div className="bg-black/40 p-4 rounded-xl border border-zinc-700">
                <div className="flex items-center gap-3 mb-2"><span className="text-2xl">📷</span><h4 className="font-bold text-white">Opción 2: Código QR</h4></div>
            ...
            <div className="mt-6 text-center">
              ...
              <a href={whatsappUrl} target="_blank" className="inline-block bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-3 rounded-xl font-bold text-sm hover:from-purple-500 hover:to-pink-500 transition-all">💬 Quiero mis tarjetas FONOTAP</a>
            </div>
          </div>
        </div>
      )}
