  ...
  return (
    <main className="min-h-screen bg-black text-white p-8">
      ...
      <div className="bg-zinc-900 p-6 rounded-xl mb-4">
        ...
        <p className="text-zinc-400">URL: {supabaseUrl || 'NO ENCONTRADA'}</p>
      ...
      <div className="bg-zinc-900 p-6 rounded-xl mb-4">
        ...
        {artistsError ? <p className="text-red-400">Error: {artistsError.message}</p> : <pre className="text-sm text-green-400">{JSON.stringify(artists, null, 2)}</pre>}
      </div>

      <div className="bg-zinc-900 p-6 rounded-xl">
        <h2 className="text-xl font-bold mb-2 text-purple-400">3. Tabla tracks:</h2>
        {tracksError ? <p className="text-red-400">Error: {tracksError.message}</p> : <pre className="text-sm text-green-400">{JSON.stringify(tracks, null, 2)}</pre>}
      </div>
    </main>
  );
}
