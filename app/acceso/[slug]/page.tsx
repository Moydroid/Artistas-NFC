  ...
  const fetchArtistData = async () => {
    try {
      ...
      if (artistError || !artistData) {
        const { data: allArtists } = await supabase.from('artists').select('id, name, slug');
        console.log('Slug buscado:', slug);
        console.log('Artistas en BD:', allArtists?.map(a => a.slug));
        setArtist(null);
      } else {
  ...
  if (!artist) return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4 text-center">
      <div>
        ...
        <p className="text-zinc-500 mb-2">El slug buscado fue: <span className="text-purple-400 font-mono">{slug}</span></p>
        <p className="text-zinc-600 text-sm">Revisa que el enlace sea correcto o contacta al administrador.</p>
      </div>
    </div>
  );
          ...
          <div className="bg-zinc-900/70 backdrop-blur-2xl border border-white/10 rounded-3xl p-5 md:p-6 shadow-2xl flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex-1 w-full text-center md:text-left space-y-4">
              <div>
                ...
                <h2 className="text-lg md:text-xl text-purple-300 font-medium">{currentTrack?.title || 'Sin titulo'}</h2>
              ...
              <div className="flex items-center justify-center md:justify-start gap-4">
                <button onClick={prevTrack} className="p-2 text-zinc-400 hover:text-white transition-colors"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z" /></svg></button>
                ...
                <button onClick={nextTrack} className="p-2 text-zinc-400 hover:text-white transition-colors"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z" /></svg></button>
                <button onClick={downloadTrack} className="p-2 text-zinc-400 hover:text-purple-400 transition-colors"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg></button>
              </div>
              <div className="space-y-1">
                ...
                <div className="flex justify-between text-xs text-zinc-400 font-mono"><span>{formatTime(progress)}</span><span>{formatTime(duration)}</span></div>
              </div>
            </div>
          </div>
                ...
                {tracks.map((track, index) => (
                  <button key={track.id} onClick={() => { setCurrentTrackIndex(index); setIsPlaying(true); }} className={`w-full p-2 rounded-lg text-left transition-all flex items-center gap-2 text-sm ${index === currentTrackIndex ? 'bg-purple-600/20 border border-purple-500/30' : 'bg-zinc-800/30 hover:bg-zinc-800/50 border border-transparent'}`}>
                    <div className="w-8 h-8 rounded bg-zinc-800 flex items-center justify-center flex-shrink-0">
                      {index === currentTrackIndex && isPlaying ? <div className="flex gap-0.5"><div className="w-0.5 h-3 bg-purple-400 animate-pulse" /><div className="w-0.5 h-3 bg-purple-400 animate-pulse" /><div className="w-0.5 h-3 bg-purple-400 animate-pulse" /></div> : <span className="text-zinc-500 text-xs">{index + 1}</span>}
                    </div>
                    <div className="flex-1 min-w-0"><p className={`font-medium truncate ${index === currentTrackIndex ? 'text-purple-300' : 'text-white'}`}>{track.title}</p></div>
                  </button>
                ))}
              </div>
            </div>
          )}
          <div className="text-center mt-4"><p className="text-xs text-zinc-500">2026 FONOTAP Contenido Exclusivo</p></div>
        </div>
      </div>
